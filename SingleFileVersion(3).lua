...
function LoaderController:FetchPlaceInfo(placeId)
    ...
    local success, result = pcall(function()
        ...
        local universeId = place.universeId
        local placeName = place.name
        
        local thumbnailUrl = "https://thumbnails.roblox.com/v1/games/multiget/thumbnails?universeIds=" .. tostring(universeId) .. "&size=768x432&format=Png&isCircular=false"
        local thumbnailResponse = game:HttpGet(thumbnailUrl)
        local thumbnailData = HttpService:JSONDecode(thumbnailResponse)
        
        local imageUrl = ""
        if thumbnailData and thumbnailData.data and #thumbnailData.data > 0 then
            local thumbnails = thumbnailData.data[1].thumbnails
            if thumbnails and #thumbnails > 0 then
                imageUrl = thumbnails[1].imageUrl
            end
        end
...
function LoaderController:CreateGUI()
    ...
    local targetPos = UDim2.new(0.5, -225, 0.5, -250)
    
    if UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled then
        targetSize = UDim2.new(0, 400, 0, 450)
        targetPos = UDim2.new(0.5, -200, 0.5, -225)
    end
