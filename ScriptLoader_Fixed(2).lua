    ...
    task.delay(index * 0.05, function()
        ...
        if index == #MAPS_CONFIG then
            ...
            if parent and parent:IsA("ScrollingFrame") then
                ...
                if layout then
                    parent.CanvasSize = UDim2.new(0, 0, 0, layout.AbsoluteContentSize.Y + 24)
                end
            end
        end
    end)
...
function LoaderController:CreateGUI()
    ...
    local function updateCanvasSize()
        ...
        ScrollFrame.CanvasSize = UDim2.new(0, 0, 0, contentHeight + 24)
    end
    ...
    local targetPos = UDim2.new(0.5, -275, 0.5, -260)
    
    if UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled then
        targetSize = UDim2.new(0, 500, 0, 480)
        targetPos = UDim2.new(0.5, -250, 0.5, -240)
    end
