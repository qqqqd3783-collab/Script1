-- Single File Version - Script Loader
...
local MAPS_CONFIG = {
    {
        ...
        thumbnailId = "rbxassetid://0", -- ใส่ Asset ID ของรูปภาพ
        scriptUrl = "https://raw.githubusercontent.com/example/script1.lua",
        color = Color3.fromRGB(96, 211, 148)
    },
    {
        name = "ESP Wallhack",
    ...
    },
    {
        ...
        thumbnailId = "rbxassetid://0",
        scriptUrl = "",
        color = Color3.fromRGB(220, 88, 88)
    },
}
...
function LoaderController:CreateNotification(text, color)
    ...
    Label.Text = text
    Label.TextColor3 = UI_CONFIG.TextPrimary
...
function LoaderController:LoadScript(mapData)
    if mapData.scriptUrl == "" or not mapData.scriptUrl then
        self:CreateNotification("❌ Script not available!", Color3.fromRGB(220, 88, 88))
        return
    end
    ...
    if success and result then
        self:CreateNotification("✓ Loading " .. mapData.name, UI_CONFIG.AccentColor)
        
        local executeSuccess, executeError = pcall(result)
        if executeSuccess then
            self:CreateNotification("✓ " .. mapData.name .. " loaded!", UI_CONFIG.AccentColor)
        else
            self:CreateNotification("❌ Execution error!", Color3.fromRGB(220, 88, 88))
            warn("Execution error:", executeError)
        end
    else
        self:CreateNotification("❌ Failed to load script!", Color3.fromRGB(220, 88, 88))
...
function LoaderController:CreateMapCard(mapData, parent, index)
    ...
    Thumbnail.Image = mapData.thumbnailId
    ...
    Status.TextColor3 = UI_CONFIG.TextSecondary
    ...
    LoadButton.BackgroundColor3 = mapData.color
    ...
    LoadButton.MouseEnter:Connect(function()
        TweenService:Create(LoadButton, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {
            ...
            BackgroundColor3 = Color3.fromRGB(
                math.min(255, mapData.color.R * 255 + 20),
                math.min(255, mapData.color.G * 255 + 20),
                math.min(255, mapData.color.B * 255 + 20)
            )
        }):Play()
